async function fetchSiteMetadata(siteUrl, requestedFieldsParam, json) {
  ...
  try {
    ...
    if (metadata.fav) {
      try {
        ...
        metadata.color = colors.color;
        metadata['c-color'] = colors['c-color'];
      } catch (error) {
    ...
    if (requestedFields) {
      metadata = requestedFields.reduce((filtered, field) => {
        if (metadata[field] !== undefined) filtered[field] = metadata[field];
        return filtered;
      }, {});
    }

    return json ? JSON.stringify(metadata, null, 2) : metadata;
  } catch (error) {
...
function getFaviconsAndOgImage(doc, baseUrl) {
  ...
  const resolveWithProxy = (url) => decodeURIComponent(new URL(url, baseUrl).href);
  ...
  const faviconsWithSize = faviconTags.map(tag => {
    ...
    const href = resolveWithProxy(tag.getAttribute('href'));
  ...
  const result = {
    ...
    'og:image': ogImageTag && !ogImageTag.content.includes('$') ? resolveWithProxy(ogImageTag.content) : ''
  };
...
async function extractColorsFromImage(imgSrc) {
  ...
  try {
    const img = await loadImage(proxyUrl(imgSrc));
    return extractColors(img);
  } catch (error) {
    console.error('Failed to load image using proxy:', error);
